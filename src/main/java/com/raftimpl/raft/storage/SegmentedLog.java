...
import com.raftimpl.raft.storage.Segment;
import java.io.File;
...
public class SegmentedLog {
 ...
 public RaftProto.LogEntry getEntry(long index){
  ...
  if(index == 0 || index < firstLogIndex || index > lastLogIndex){
   LOG.debug("index out of range, index={},firstLogIndex={},lastLogIndex={}",index,firstLogIndex,lastLogIndex);
   return null;
  }
  if(startLogIndexSegmentMap.size()==0){
   return null;
...
/////////
 public long append(List<RaftProto.LogEntry> entries){
  long newLastLogIndex= this.getLastLogIndex(); // 获取当前日志的最后索引
  for(RaftProto.LogEntry entry: entries){// 遍历每个待追加的日志条目
   newLastLogIndex++;// 增加新日志的索引
   int entrySize= entry.getSerializedSize();// 获取日志条目的序列化大小
   int segmentSize = startLogIndexSegmentMap.size();// 获取当前段的数量
   boolean isNeedNewSegmentFile=false;
   try{
    if(segmentSize ==0){// 如果当前没有段文件
     isNeedNewSegmentFile =true;// 需要新建段文件
    }else{
    ...
    if(entry.getIndex()==0){
     entry = RaftProto.LogEntry.newBuilder(entry).setIndex(newLastLogIndex).build(); // 设置新的索引并构建新的日志条目
 ...
 //从日志中删除指定索引之前的所有日志条目，以释放存储空间并保持日志文件的大小在合理范围内。
...
//删除指定索引之后的所有日志条目，以保持日志文件的大小在合理范围内，并确保日志条目只保留到指定的结束索引。
public void truncateSuffix(long newEndIndex){
 ...
 public RaftProto.LogMetaData readMetaData() {
  ...
  try (RandomAccessFile randomAccessFile = new RandomAccessFile(file, "r")) {
   RaftProto.LogMetaData metadata = RaftFileUtils.readProtoFromFile(randomAccessFile, RaftProto.LogMetaData.class);
   return metadata;
